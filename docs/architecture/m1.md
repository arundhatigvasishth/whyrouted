...
- `HttpReplicaAdapter` (`src/adapter/http.ts`) implements it against real HTTP:
  `fetch` with `AbortController` timeouts, id-to-URL resolution from the config
...
|---|---|---|
| Health Scheduler writes to Service Registry | `scheduler` writes through `HealthSink` (a `Pick<RegistryStore, "updateRuntime" \| "setHealth">`), so it doesn't even see the whole registry | ✅ |
...
- `/status` is one read route. No auth, no query params, no other endpoints. The
  request-serving surface is M2.
...
3. **All replicas launch with `DEFAULT_PROFILE`.** `synthetic.ts` supports
   ...
   through. M2 load-distribution testing (success metric: no replica > 1.5x
...
## 5. Assumptions from [`../milestones/m1/fleet-foundation.md`](../milestones/m1/fleet-foundation.md): still open, need live data
...
- [x] Replica & health track walked and agreed (Junaid)
- [x] Clear to start M2 (both)
