...
**Why:** the only M1 implementation is an in-memory `Map` — instant. Making the
interface async now would push `await` into the health scheduler, the `/status`
handler, and `main.ts` for no M1 benefit.
...
**Revisit when:** M8 (deployment) — each component becomes its own K8s deployment.
...
**Revisit when:** M2 load-distribution testing, if a replica exceeds 1.5× median
...
## 2026-09-01 — hysteresis thresholds: N=3 fail, M=2 recover, 1s interval
