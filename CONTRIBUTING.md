...
(milestone task splits, idea docs, this file) land directly on `main`. They are
not code to review.
...
Applies to commit messages, PR descriptions, docs, and code comments.

- **No em dashes.** Don't use `—` (or `–`) as a punctuation break. Use a comma,
  a colon, parentheses, or two sentences instead. A hyphen between a number range
  (`8001-8004`) or in a compound word is fine.
- Plain, direct prose. Short sentences. No filler.
- Lowercase for commit messages and branch names.
...
- **Lots of small commits.** Each commit is one coherent step that builds and,
  ...
  line, it's doing too much, so split it.
...
- Load-bearing technical decisions get recorded in `docs/decisions.md` as they
  are made.
